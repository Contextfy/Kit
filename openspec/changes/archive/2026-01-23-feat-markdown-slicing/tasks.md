...
### Task-01: 设置与测试框架
- [x] 1.1 在 `packages/core/src/parser/mod.rs` 中定义 `SlicedDoc<'a>` 结构体
  - 字段：`pub section_title: String`, `pub content: &'a str`, `pub parent_doc_title: &'a str`
  - 确保所有字段为公开，以便其他模块使用
  - 使用生命周期参数实现零拷贝
...
- [x] 1.3 编写测试用例（覆盖边界情况）：
  ...
  - **测试 5 - 空内容**：连续的 `##` 标题 → 前一个分片内容为空
  - **测试 6 - Unicode**：包含中文和 Emoji 的标题与内容 → 字节切片不 Panic
  - **测试 7 - 内联格式**：H2 标题包含粗体/斜体等 Markdown 格式 → 完整捕获标题文本
  - **测试 8 - 行内代码和换行**：H2 标题包含 `` `code` `` 和换行符 → 正确处理
  - **测试 9 - 零拷贝验证**：验证 `content` 和 `parent_doc_title` 是借用切片

### Task-02: 实现切片逻辑
- [x] 2.1 在 `packages/core/src/parser/mod.rs` 中实现 `pub fn slice_by_headers<'a>(content: &'a str, parent_title: &'a str) -> Vec<SlicedDoc<'a>>`
- [x] 2.2 使用 `pulldown-cmark` crate 遍历 markdown 事件
- [x] 2.3 检测 `Event::Start(Tag::Heading(HeadingLevel::H2, ...))` 事件
- [x] 2.4 捕获 H2 标题之间的内容
- [x] 2.5 处理边界情况：
  - 忽略第一个 H2 之前的内容
  - 将 H3/H4 标题作为当前 H2 分片内容的一部分
  - 无 H2 标题时返回空向量
...
- [x] ✅ 所有单元测试通过（9/9 切片测试 + 2/2 解析测试）
- [x] ✅ 代码通过 clippy 检查（无警告）
- [x] ✅ 代码格式化完成
- [x] ✅ 支持带内联格式的 H2 标题（如 `## Section **One**`）
- [x] ✅ 真正的零拷贝实现（使用 `&'a str` 借用切片）
- [x] ✅ 支持行内代码（如 `## Use `->` operator`）

### 实现亮点
- ✅ **真正的零拷贝实现**：`content` 和 `parent_doc_title` 使用借用切片 `&'a str`
- ✅ 使用 `into_offset_iter()` 获取字节范围，直接在原始字符串上切片
- ✅ 完整的 AST 解析，正确处理代码块中的 `##`
- ✅ Unicode 和 Emoji 支持，无 Panic
- ✅ TDD 驱动开发，红-绿-重构循环
- ✅ 支持带内联格式的 H2 标题（通过 Event::End 确保完整捕获）
- ✅ 支持行内代码（Event::Code）和换行处理（Event::SoftBreak/HardBreak）
