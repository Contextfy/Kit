...
- **WHEN** 开发者查看 `packages/cli/src/` 目录
- **THEN** 系统应包含：
  - `main.rs` - CLI 入口和命令分发
  - `commands/` - 命令实现目录
    - `mod.rs` - 命令模块入口
...
- **WHEN** 用户执行 `contextfy build` 命令且 `contextfy.json` 包含 `docs_path` 配置
- **THEN** 系统从配置文件读取 `docs_path` 字段作为文档源目录
- **AND** 使用该目录扫描 Markdown 文件进行解析和存储

#### Scenario: 回退到默认路径

- **WHEN** 用户执行 `contextfy build` 命令且 `contextfy.json` 不存在或不包含 `docs_path` 字段
- **THEN** 系统回退到默认路径 `docs/examples` 作为文档源目录
- **AND** 在控制台输出提示信息指明使用的路径

#### Scenario: contextfy.json 配置结构

- **WHEN** 系统读取 `contextfy.json` 配置文件
- **THEN** 配置文件应包含以下结构：
  ```json
  {
    "name": "项目名称",
  ...
  ```
- **AND** `docs_path` 字段为可选字段

#### Scenario: 配置路径不存在时错误提示

- **WHEN** `contextfy.json` 中配置的 `docs_path` 指向不存在的目录
- **THEN** 系统返回描述性错误信息
- **AND** 错误信息包含配置的路径和提示用户检查配置或运行 `contextfy init`
